...
def process_latest_stream_pair():
    ...
    if not (has_new_a and has_new_b):
        st.session_state.last_update_message = "No new image pair yet."
        return st.session_state.latest_result
...
def show_pair_images(item):
    ...
    with col2:
        st.markdown("**Wavelet Image / YOLO Input**")
        img_b = item.get("stream_image_b_path")
...
with right_stats:
    st.markdown('<div class="panel">', unsafe_allow_html=True)
    st.markdown('<div class="panel-title">Fusion Output</div>', unsafe_allow_html=True)
    ...
    if latest:
        ...
        <div class="metric-side">
            <div class="metric-side-number">{g_val:.4f}</div>
            <div class="metric-side-label">Fusion Probability (G)</div>
        </div>
        """, unsafe_allow_html=True)
    else:
        st.info("No fusion output available yet.")
